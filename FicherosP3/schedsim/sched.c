...
static void load_balance (int this_cpu,int simulation_step)
{
	...
	if (nr_cpus==1 || this_rq->next_load_balancing!=simulation_step) {  //Si solo hay una cpu, o en es momento de hacer balancing, hacemo return
		unlock_rq(this_rq);
		return;
	}
	...
	while (!load_balanced_system && (nr_trials<max_trials)) {
		...
		//Si la diferencia entre la cpu con más carga y la de menos es solo de 1, el sistema está balanceado
		if (max_load-min_load<=1) {
			load_balanced_system=TRUE;
		} else if (load_cpus[this_cpu]==max_load) { //Estamos en la cpu con más carga
		...
		} else if (load_cpus[this_cpu]==min_load) { //Estamos en la cpu con menos carga
