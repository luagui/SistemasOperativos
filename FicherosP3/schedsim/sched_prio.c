...
static void enqueue_task_prio(task_t* t,runqueue_t* rq, int preempted)
{
	...
	if (t->flags & TF_INSERT_FRONT) {
		...
		sorted_insert_slist_front(&rq->tasks, t, 0, compare_tasks_priority);  //Push task
	} else
		sorted_insert_slist(&rq->tasks, t, 0, compare_tasks_priority);  //Push task
...
{
	//Devolvemos la cola, ya que será la task con menor prioridad
