...
int resizeNode(uint64_t idxNode, size_t newSize) {
	...
	if(!diff) //Si la nueva size es igual
		return 0;

	memset(block, 0, sizeof(char)*BLOCK_SIZE_BYTES); //Rellenamos block con 0 reservando memoria para la variable
	...
	if(diff > 0) {
		...
		if(node->numBlocks && node->fileSize % BLOCK_SIZE_BYTES) {
			int currentBlock = node->blocks[node->numBlocks - 1];  //currentBlock es el numero de bloque en nuestro SF
            
            if( readBlock(&myFileSystem, currentBlock, &block)==-1 ){ //Guardamos el ultimo bloque sdel archivo en block.
                fprintf(stderr,"Error reading block in resizeNode\n");
                return -EIO;
            }
			
			int offBlock = node->fileSize % BLOCK_SIZE_BYTES; //Numero de bytes en el ultimo bloque
		...
		if(newBlocks) {
			...
			for(i = 0; currentBlock != node->numBlocks; i++) { //Recorremos el bitMapl para asignar numero de bloque a los nuevos bloques 
	...
	/// File decreases
		...
		// Bloques con la nuevo tamaño(redondeo hacia arriba)
		...
		for(i = node->numBlocks; i > numBlocks; i--) {
			...
			myFileSystem.bitMap[nBloque] = 0; //Desmarcamos los bloques que se pierden por la reduccion de tamaño
		...
		node->fileSize += diff; //Diff es negativo en este caso
	}
...
static int my_write(const char *path, const char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
	...
	while(bytes2Write) {
        ...
        if( readBlock(&myFileSystem, currentBlock, &buffer)==-1 ){  //Guardamos el bloque donde tenemos que empezar a escribir en buffer
		...
		}   //Completamos el buffer con lo que queramos escribir (buf)
...
static int my_mknod(const char *path, mode_t mode, dev_t device) {
	...
	if(myFileSystem.nodes[idxNodoI] == NULL) //Reservamos memoria si el nodo estaba a null
	...
	reserveBlocksForNodes(&myFileSystem, myFileSystem.nodes[idxNodoI]->blocks, 0); //Reservamos 0 bloques por defecto.
	//Al escribir (my_write), si no hay bloques suficientes, ya se reservan bloque 
