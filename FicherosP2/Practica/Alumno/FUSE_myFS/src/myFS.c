...
int findFreeNode(MyFileSystem* myFileSystem) {
 ...
 //Nos devuelve la posicion del inode en bytes a partir de su nodeNum ( numero en el array de punteros a nodo de FileSystem)
int findNodeByPos(int nodeNum) {
	...
	//Empezamos en el bloque NODES_IDX, ya que antes estan los bloques 0, 1 y 2 ocupados por Superblock, bitMap, rootDirectory
...
int initializeNodes(MyFileSystem *myFileSystem) {
	...
	for(numNode = 0; numNode < MAX_NODES; numNode++) {
		if(readNode(myFileSystem, numNode, &temp)) //Read node devuelve 0 (false) si ha sido OK
			return -1;
		// Si el nodo que hemos leido esta libre, guardamos null
		if(temp.freeNode) {
			myFileSystem->nodes[numNode] = NULL;
		}
		// Hacemos hueco en memoria para los inodos que hemos leido de nuestro fichero fisico
		else {
			if( (myFileSystem->nodes[numNode] = malloc(sizeof(NodeStruct)))==NULL){
				perror("Error in malloc");
				return -2;
			}
			copyNode(myFileSystem->nodes[numNode], &temp); //(dest, origen)
...
int myMkfs(MyFileSystem *myFileSystem, int diskSize, char *backupFileName) {
	...
	int minNumBlocks = 3 + MAX_BLOCKS_WITH_NODES + 1; //Bloques para Superblock, directory y bitMap, bloques para los inodes, y al menos un bloque de datos
	int maxNumBlocks = NUM_BITS;  //Justo el tamaño del array bitMap
	...
	//Escribimos todos los cambios en nuestro fichero fisico
	...
	for(i = 0; i < MAX_NODES; i++) {
		updateNode(myFileSystem, i, &currentNode); //Ponemos todos los nodos como libres en el archivo real
	}
	...
	//Leemos los nodos del archivo real y reservamos memoria en el array *nodes [] para aquellos que tengan freeNode != 1. 
	//En nuestro caso, acabamos de escribir en el fichero real todos los inodos como libres, luego 
	//no reservamos memoria, simplemente inicializará el array *nodes[] a null
...
int readNode(MyFileSystem *myFileSystem, int nodeNum, NodeStruct* node) {
	...
	//Reposicionamos el offset de nuestro archivo, , myFileSystem->fdVirtualDisk (es un fd),
	//a la posicion posNode si devuelve -1, hemos leido más alla de eof. (lseek)

	//Si no leemos sizeof(NodeStruct) bytes, error (read)
...
//Escribimos en el archivo fisico
...
//Escribimos el nodo en el fichero fisico
